...
#Bullet spawn
const bulletScene = preload("res://staticSpreadEnemy/enemy_spread_bullet.tscn")
...
func _ready():
	...
	for i in range(spawnPointCount):
		var spawnPoint = Node2D.new()
		#offset by half a step for alternate pattern; 1.2 pushes it slightly ahead of the first row
		var pos = Vector2(radius * 1.2, 0).rotated(step * i + step / 2)
		spawnPoint.position = pos
		spawnPoint.rotation = pos.angle()
		rotater.add_child(spawnPoint)
